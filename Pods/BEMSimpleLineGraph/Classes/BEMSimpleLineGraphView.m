...
- (void)drawGraph {
    ...
    float maxValue = 100.0; // Biggest Y-axis value from all the points.
    float minValue = 0.0; // Smallest Y-axis value from all the points.
    ...
    for (int i = 0; i < numberOfPoints; i++) {
        ...
        positionOnYAxis = (self.frame.size.height - padding) - ((dotValue - minValue) / ((maxValue - minValue) / (self.frame.size.height - padding))) + 20;
            ...
            positionOnYAxis = 61 - positionOnYAxis;
            
            NSLog(@"\nPosition on Y Axis: %.3lf\n", positionOnYAxis);
        }
    ...
    for (int i = 0; i < numberOfPoints - 1; i++) {
        ...
        NSLog(@"\n\nThe Frame: %@\n", NSStringFromCGRect(CGRectMake(0, 0, self.viewForBaselineLayout.frame.size.width, self.viewForBaselineLayout.frame.size.height)));
